...
class VideoRecord(object):
    def __init__(self, row, modality):
        #print(row)
        self._data = row
        #print(int(self._data[1]))
        #sys.exit()
        self._modality = modality
    @property
    def path(self):
        core_path = self._data[0] 
        #print(core_path)
        #sys.exit()
        return core_path
    ...
    def num_frames(self):
        if self._modality == 'Flow':
            #print(os.path.join('/home/user/thumos/hmdb51_opticalflow', self._data[0][]))
            #print(self._data[0])
            images = glob.glob(os.path.join('/root/data/UCF-101_rgb_flow', self._data[0], 'flow_x_*.jpg'))
            n_f = len(images)
            #print(n_f)
            return int(n_f)
        images = glob.glob(os.path.join('/root/data/UCF-101_rgb_flow', self._data[0], 'image_*.jpg'))
        n_f = len(images)
        #print(n_f)
        return int(n_f)
...
class TSNDataSet(data.Dataset):
    ...
    def _load_image(self, directory, idx):
        ...
        if self.modality == 'RGB' or self.modality == 'RGBDiff':
            ...
            flow_tmpl = "{}_{:05d}.jpg"
            #print(os.path.join(self.root_path, directory, flow_tmpl.format('flow_x', idx)))
            x_img = Image.open(os.path.join(self.root_path, directory, flow_tmpl.format('flow_x', idx))).convert('L')
            #print(os.path.join(self.root_path, directory, flow_tmpl.format('flow_y', idx)))
            y_img = Image.open(os.path.join(self.root_path, directory, flow_tmpl.format('flow_y', idx))).convert('L')
            #sys.exit()
            return [x_img, y_img]
            #return x_img
