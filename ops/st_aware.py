...
class SeqVLADModule(torch.nn.Module):
    ...
    def __init__(self, in_channels, timesteps, num_centers, redu_dim, with_relu=False, activation=None, with_center_loss=False, init_method='xavier_normal'):
        '''
        ...
        self.center_dim = num_centers
        self.redu_dim = redu_dim
        self.timesteps = timesteps
        self.with_relu = with_relu
        self.in_channels = in_channels
        # print('## in SeqVLADModule ##',self.num_centers, self.redu_dim)
        self.inter_channels = redu_dim //2

        self.in_shape = None
        self.out_shape = self.center_dim*self.redu_dim
        ...
        print("ccc = " + str(self.in_channels))
        print("ccc = " + str(self.redu_dim))
        self.redu_w = torch.Tensor(self.redu_dim, self.in_channels, 3, 3, 3) # weight : out, in , h, w
        self.redu_w = torch.nn.init.xavier_normal_(self.redu_w) 
        self.redu_w = torch.nn.Parameter(self.redu_w, requires_grad=True)
 
        self.redu_wc = torch.Tensor(self.center_dim, self.redu_dim, 3, 3, 3) # weight : out, in , h, w
        self.redu_wc = torch.nn.init.xavier_normal_(self.redu_wc) 
        self.redu_wc = torch.nn.Parameter(self.redu_wc, requires_grad=True)
        self.centers = torch.Tensor(self.center_dim, self.redu_dim)
        self.centers = torch.nn.init.xavier_normal_(self.centers)
        self.centers = torch.nn.Parameter(self.centers, requires_grad=True)


        self.redu_b = torch.Tensor(self.redu_dim,)
        self.redu_b = torch.nn.init.uniform_(self.redu_b)
        self.redu_b = torch.nn.Parameter(self.redu_b, requires_grad=True)

        self.redu_kb = torch.Tensor(self.center_dim,)
        self.redu_kb = torch.nn.init.uniform_(self.redu_kb)
        self.redu_kb = torch.nn.Parameter(self.redu_kb, requires_grad=True)
    ...
    def forward(self, input):
        ...
        # return VideoSeqvlad(self.timesteps, self.center_dim, self.redu_dim)(input)

        #input_tensor: N*timesteps, C, H, W
        self.in_shape = input.size()
        slice_dim =self.in_shape[0]
        self.batch_size = self.in_shape[0]//self.timesteps
        #print("self.in_shape: {}".format(input.size()))
        if self.batch_size == 0:
            self.batch_size = 1
        input = input.view(self.batch_size, self.in_shape[1], self.timesteps, self.in_shape[2], self.in_shape[3])
        #print("self.in_shape: {}".format(input.size()))
        ...
        input_tensor = torch.nn.functional.conv3d(input, self.redu_w, bias=self.redu_b, stride=1, padding=1)
        assignments = torch.nn.functional.conv3d(input_tensor, self.redu_wc, bias=self.redu_kb, stride=1, padding=1)

        ## timesteps, batch_size, center_dim, h, w ==> batch_size, timesteps, center_dim, h, w
        assignments = torch.transpose(assignments, 0, 1).contiguous()
        #print('transposed assignments shape', assignments.size())

        ## assignments: batch_size, timesteps, center_dim, h*w
        assignments = assignments.view(self.batch_size*self.timesteps, self.center_dim, self.in_shape[2]*self.in_shape[3])
        ...
        if self.activation is not None:
            if self.activation == 'softmax':
                ...
                assignments = assignments.view(self.batch_size*self.timesteps*self.in_shape[2]*self.in_shape[3], self.center_dim)
        ...
        ## alpha *c 
        ## a_sum: batch_size, timesteps, center_dim, 1
        a_sum = torch.sum(assignments, -1, keepdim=True)

        ## a: batch_size*timesteps, center_dim, redu_dim
        a = a_sum * self.centers.view(1, self.center_dim, self.redu_dim)

        center_tmp = self.centers.view(-1, self.center_dim*self.redu_dim)
        #print("centers:{}".format(center_tmp.shape))
        ## alpha* input_tensor
        ## fea_assign: batch_size, timesteps, center_dim, h, w ==> batch_size*timesteps, center_dim, h*w 
        # fea_assign = assignments.view(self.batch_size*self.timesteps, self.center_dim, self.in_shape[2]*self.in_shape[3])
        ...
        ## x: batch_size*timesteps, center_dim, redu_dim
        x  = torch.matmul(assignments, input_tensor)


        ## batch_size*timesteps, center_dim, redu_dim
        vlad = x - a 
        
        #print("vlad shape:{}".format(vlad.shape))

        ## batch_size*timesteps, center_dim, redu_dim ==> batch_size, timesteps, center_dim, redu_dim
        vlad = vlad.view(self.batch_size, self.timesteps, self.center_dim, self.redu_dim)  # sigma Timestep
        #print("vlad3 shape:{}".format(vlad.shape))

        ## batch_size, center_dim, redu_dim 
        vlad = torch.sum(vlad, 1, keepdim=False)

        ## intor normalize
        vlad = torch.nn.functional.normalize(vlad, p=2, dim=2)
        
        #print("vlad2 shape:{}".format(vlad.shape))
        ## l2-normalize
        vlad = vlad.view(self.batch_size, self.center_dim*self.redu_dim)
        ...
        if not self.with_center_loss:
            ...
            #print("assign_predict:{}".format(assign_predict.shape))
            return assign_predict, vlad

class BiSeqVLADModule(torch.nn.Module):

    def __init__(self, timesteps, center_dim, redu_dim, with_relu=False, activation=None):
        '''
            num_centers: set the number of centers for sevlad
            redu_dim: reduce channels for input tensor
        '''
