...
from seqvlad_models import SeqVLAD, SeqVLAD_with_centerloss
...
def main():
    ...
    if args.optim == 'SGD':
        ...
        optimizer_cent = torch.optim.SGD(policies, args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
        if args.two_steps is not None:
            sub_optimizer = torch.optim.SGD(sub_policies, args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
            sub_optimizer_cent = torch.optim.SGD(sub_policies, args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.two_steps is not None and epoch < args.two_steps:
         #   validate(val_loader, model, criterion,criterion_cent, 0)
            ...
            train(train_loader, model, criterion, sub_optimizer, optimizer_cent, epoch)
        else:       
            #print("global optimiazer!!!!!")
            adjust_learning_rate(optimizer, epoch, args.lr_steps)
            #adjust_learning_rate(optimizer_cent, epoch, args.lr_steps)
            train(train_loader, model, criterion, optimizer, optimizer_cent, epoch)
...
def validate(val_loader, model, criterion, iter, logger=None):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        #print("tar  shape {}".format(target_var.shape))
        loss = criterion(output, target_var)
        #loss_cent = criterion_cent(feature, target_var)
        #loss_cent = 0.1 * loss_cent
        #loss = loss_cent
        # measure accuracy and record loss
        prec1, prec5 = accuracy(output.data, target, topk=(1,5))
