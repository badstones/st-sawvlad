...
from ops.st_aware import ConsensusModule, Identity, SeqVLADModule, BiSeqVLADModule, UnshareBiSeqVLADModule
from ops.basic_ops1 import SeqVLADUniformModule
from transforms import *
from torch.nn.init import normal, constant
from bninception import bninception
from nlSecondordervlad import SecondordVlad
import sys
#from non_localmodule import NonLocalModule
...
SeqVLAD Configurations:
    ...
    init_method:        {}
        ...
        #self.feature = self.base_model.features
    ...
    def _add_seqvlad_layer(self, base_model):
        if base_model == 'BNInception':
            ...
            if self.seqvlad_type == 'seqvlad':
                setattr(self.base_model, 'avepool', 
                        SeqVLADModule(1024, self.timesteps, self.num_centers, self.redu_dim, self.with_relu, self.activation, init_method=self.init_method))
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv1d):
                ...
                if self.redu_dim < 1024:
                    #print("...", self.redu_dum)
                    seq_count = 2
                    assert len(list(m.parameters())) == 5, "the number parameters of seqvlad should be equal to 12"
                else:
                    seq_count = 4
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv1d):
                ...
                if self.redu_dim < 1024:
                    ...
                    seq_count=4
                    assert len(list(m.parameters())) == 6, "the number parameters of seqvlad should be equal to 9"
                conv_cnt += seq_count
                #print(ps[0:2])
                #sys.exit()
                normal_weight.extend(ps[0:seq_count])
...
SeqVLAD Configurations:
    ...
    def _add_seqvlad_layer(self, base_model):
        if base_model == 'BNInception':
            ...
            #self.global_pool = SeqVLADModule(self.timesteps, self.num_centers, self.redu_dim)
            setattr(self.base_model, 'global_pool', 
                    SeqVLADModule(1024,self.timesteps, self.num_centers, self.redu_dim, self.with_relu, self.activation))
            #self.base_model = model
...
class SeqVLAD_with_centerloss(nn.Module):
    def __init__(self, num_class, num_centers, modality,
        ...
        self.with_relu = with_relu
        self.seqvlad_type = seqvlad_type
        self.register_buffer('centers',(torch.rand(num_class, redu_dim * num_centers).cuda()-0.5)*2)        
...
dropout_ratio:      {}
        ...
        #self._add_seqvlad_layer(base_model)
        self._add_non_local(base_model)
    
        #sona = SecondordVlad(self.num_centers, self.redu_dim, 2).cuda()
        #self._addsecordvlad_layer(base_model)
    ...
    def _add_non_local(self, base_model):
        if base_model == 'BNInception':
            setattr(self.base_model,'avepool', Network(1024, 32))
    ...
    def _add_seqvlad_layer(self, base_model):
        if base_model == 'BNInception':
            ...
            setattr(self.base_model, 'avepool',
                    SeqVLADModule(1024,self.timesteps,
                    self.num_centers, self.redu_dim,
                    self.with_relu, self.activation, 
    ...
    #def _addsecordvlad_layer(self, base_model):
    #    self.secordvlad = SecondordVlad(self.num_centers, self.redu_dim, 1)
    ...
    def _add_classifier_layer(self, base_model, num_class):
        if base_model == 'BNInception':
            ...
            if self.dropout == 0:
                ...
                self.new_fc = nn.Linear(feature_dim, num_class)
                #self.test_fc =  nn.Linear(feature_dim, 64)
                #self.new_fc = nn.Linear(64, num_class)

                setattr(self.base_model, 'last_linear', None)
            std = 0.001
            normal(self.new_fc.weight, 0, std)
            constant(self.new_fc.bias, 0)
            return feature_dim
    ...
    def get_sub_optim_policies(self):
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv1d):
                ...
                print('this is SeqVlad module, and adding the trainable parameters to train')
                assert len(list(m.parameters())) == 5, "the number parameters of seqvlad should be equal to 8"
                ps = list(m.parameters())
                conv_cnt += 5
                normal_weight.extend(ps[0:5])
                normal_bias.extend(ps[5::])
                print('len of weight %d' %(len(ps[0:5])))
                print('len of bias %d' %(len(ps[5::])))
            elif isinstance(m, SecondordVlad):
                print('this is SecondordVlad module,and adding the trainable parameters to train')
                #print("len sec para :{}".format(len(list(m.parameters()))))
                #sys.exit()
                #assert len(list(m.parameters())) == 4
                #ps = list(m.parameters())
                #conv_cnt += 2
                #normal_weight.extend(ps[0:2])
                #normal_bias.extend(ps[2::])
            elif isinstance(m, UnshareBiSeqVLADModule):
                print('this is unshare SeqVlad module, and adding the trainable parameters to train')
                assert len(list(m.parameters())) == 13, "the number parameters of seqvlad should be equal to 13"
                ps = list(m.parameters())
    ...
    def get_optim_policies(self):
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv1d):
                ...
                print('this is SecondordVlad module')
                #assert len(list(m.parameters())) == 4
        ...
        return [
            {'params': first_conv_weight, 'lr_mult': 2 if self.modality == 'Flow' else 1, 'decay_mult': 1,
             'name': "first_conv_weight"},
            {'params': first_conv_bias, 'lr_mult': 10 if self.modality == 'Flow' else 2, 'decay_mult': 0,
             'name': "first_conv_bias"},
            {'params': normal_weight, 'lr_mult': 1, 'decay_mult': 0,
             'name': "normal_weight"},
    ...
    def forward(self, input):
        sample_len = (3 if self.modality =="RGB" else 2)* self.new_length
        out= self.base_model(input.view((-1, sample_len) + input.size()[-2:]))
        #print(out.shape)
        if self.dropout > 0:
            out = self.dropout_layer(out)
        output = self.new_fc(out)
        return output 
    ...
    def forward(self, input):
        ...
        # print('shape', input.view((-1, sample_len) + input.size()[-2:]).size())
        # print('type', type(input.view((-1, sample_len) + input.size()[-2:])))
        assign_predict, vlad_out = self.base_model(input.view((-1, sample_len) + input.size()[-2:]))
        #assign_predict, vlad_out = self.seqvlad_layer(base_out)
        vlad_temp = vlad_out.view(-1, self.num_centers, self.redu_dim)

        vlad_out = torch.nn.functional.normalize(vlad_temp, p=2, dim=2)
        
        #secordvlad = self.secordvlad(vlad_temp)

        #secordvlad = torch.nn.functional.normalize(secordvlad, p=2, dim=2)
        
        #vec = vlad_out + secordvlad
        vec = vlad_out
         
        vec = vec.view(-1, self.num_centers*self.redu_dim)
        vec = torch.nn.functional.normalize(vec, p=2, dim=1)
    
        #secordvlad = secordvlad.view(-1, self.num_centers*self.redu_dim)
        #secordvlad = torch.nn.functional.normalize(secordvlad, p=2,dim=1)

        print("vec : {}".format(vec.shape))

        #vlad_out = torch.cat((vlad_temp, secordvlad), 2)
        
        #vlad_out = torch.nn.functional.normalize(vlad_out, p=2,dim=1)
        #print("vlad :{}".format(vlad_out.shape))

        #vlad_out = vlad_out.view(-1, self.num_centers*self.redu_dim * 2)
        #print(vlad_out.shape)

        if self.dropout > 0:
            vlad_out = self.dropout_layer(vec)
        
        #print(vlad_out.shape)
        #vlad_out = self.test_fc(vlad_out)
        output = self.new_fc(vec)

        if not self.before_softmax:
            output = self.softmax(output)
        
        return  vec, output.squeeze(1)
    '''
    def _get_diff(self, input, keep_rgb=False):
        input_c = 3 if self.modality in ["RGB", "RGBDiff"] else 2
        input_view = input.view((-1, self.num_segments, self.new_length + 1, input_c,) + input.size()[2:])
